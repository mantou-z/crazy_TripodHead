...
def main(opts):
    ...
    #     def get_next_frame(_): return obj.read()

    # else:
    #     obj = my_queue(os.listdir(obj_name))
    #     def get_next_frame(_): return obj.pop_front()
    obj, get_next_frame = None, None  # init obj
    ...
    3. start tracking
    """
    tracker = TRACKER_DICT[opts.tracker](
        opts, frame_rate=30, gamma=opts.gamma)  # instantiate tracker  TODO: finish init params
    ...
    while True:
        ...
        for trk in current_tracks:
            ...
            if bbox[2] * bbox[3] > opts.min_area:
                ...
                cur_id.append(id)
                cur_cls.append(cls)
                # zdx_try send the box center
                center_point = [bbox[1]+bbox[2]/2, bbox[0] +
                                bbox[3]/2]  # [center_x,center_y]
                send_buffer[1] = (int(center_point[1]) >> 8) & 0xff
                send_buffer[2] = (int(center_point[1])) & 0xff
                send_buffer[3] = (int(center_point[0]) >> 8) & 0xff
                send_buffer[4] = (int(center_point[0])) & 0xff
                Serial_try.send(send_buffer)
                # results.append((frame_id + 1, id, bbox, cls))

        results.append((frame_id + 1, cur_id, cur_tlwh, cur_cls))
        timer.toc()  # end timing this image

        plot_img(img0, frame_id, [cur_tlwh, cur_id, cur_cls], save_dir=os.path.join(
            SAVE_FOLDER, 'reuslt_images', obj_name))
...
def resize_a_frame(frame, target_size):
    ...
    frame_resized = cv2.resize(
        frame, (target_size[0], target_size[1]))  # (H', W', C)
...
def plot_img(img, frame_id, results, save_dir):
    ...
    cv2.imshow("results", img_)
    cv2.waitKey(1)
    cv2.imwrite(filename=os.path.join(
        save_dir, f'{frame_id:05d}.jpg'), img=img_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--obj', type=str, default='0',
                        help='video NAME or images FOLDER NAME')

    parser.add_argument('--save_txt', type=bool,
                        default=False, help='whether save txt')

    parser.add_argument('--save_video', type=bool,
                        default=False, help='whether save video')

    parser.add_argument('--tracker', type=str,
                        default='bytetrack', help='sort, deepsort, etc')

    parser.add_argument('--model_path', type=str,
                        default='./weights/yolov5s.pt', help='model path')

    parser.add_argument('--trace', type=bool, default=False,
                        help='traced model of YOLO v7')
    ...
    # open the serial  zdx_try
    Serial_try.port_open_recv("/dev/ttyUSB0", 115200)
